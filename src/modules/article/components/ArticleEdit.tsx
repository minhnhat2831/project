...
export default function ArticleEdit({ open, setOpen, article }: props) {
        ...
        useForm<ArticleForm>({
            ...
            values: articleId
                ? {
                    ...
                    timeToRead: articleId?.timeToRead ?? 1,
                    content: articleId?.content ?? "",
                    picture : articleId?.picture?.uri ?? ""
                }
                : undefined
        })
    ...
    const onSubmit = async (data: ArticleForm) => {
        try {
            ...
            const response = await EditArticle(article.id, submitData);
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col h-full overflow-auto">
            <div className="py-2 px-2 h-6/9 flex-1 overflow-auto">
                ...
                <Image
                    ...
                    defaultImage={articleId?.picture?.uri ?? article?.picture?.uri ?? ""}
                />
