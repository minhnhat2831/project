...
export default function CashTransactionFormDetail() {
    ...
    function autoDescription(watchType: string | null) {
        ...
        if (!isOther && watchType) {
            setValue("data.description", formatTransactionType(watchType))
        } else if (isOther) {
    ...
    return (
        <>
            ...
            ${open ? (watchType ? "h-auto" : "h-50") : "h-12"}`}>
                ...
                <div
                    className={`px-4 transition-all duration-400 ease-in-out 
                         ${open ? (watchType ? (watchIsin ? "h-auto pt-4 overflow-hidden" : "h-auto pt-4") : "h-100 pt-2") : "h-0 pt-0 overflow-auto"}`}
                >
                    <div className="flex mb-4 py-2">
                        <label className="w-50 mt-1 shrink-0">Transaction Type<span className="text-red-500"> *</span></label>
                        <SelectForm
                            name="data.transactionType"
                            control={control}
                            options={typeMode === "Debit" ? transactionTypeDebit : transactionTypeCredit}
                            onValueChange={(value) => autoDescription(value)}
                            error={get(errors, "data.transactionType.message")}
                        />
                    </div>
                    <div className="flex">
                        <label className="w-50 mt-1 shrink-0">Transaction Status</label>
                        <ButtonStatus />
                    </div>
                    {renderModal()}
                </div>
